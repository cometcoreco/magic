	...
	<div class="relative">
		<div class="container mb-28">
			<div class="w-1/2 mx-auto text-center max-lg:w-10/12 max-sm:w-full">
				<p class="text-xs font-semibold tracking-widest uppercase mb-9"><span class="inline-block px-3 py-1 !me-2 rounded-xl bg-[#262626]">{{$setting->site_name}}</span> {{$fSetting->footer_text_small}}</p>
				<p class="text-[100px] font-bold font-oneset leading-none tracking-tight mb-8 text-transparent bg-clip-text bg-gradient-to-br from-transparent -from-[5%] to-white to-50% max-sm:text-[18vw]">{{$fSetting->footer_header}}</p>
				<p class="text-[20px] font-oneset leading-[1.25em] opacity-50 font-normal mb-9 px-10">{{$fSetting->footer_text}}</p>
				<x-button link="{{$fSetting->footer_button_url}}" label="{{$fSetting->footer_button_text}}" size="lg" iconPos="end" bg="bg-white bg-opacity-10 border-[2px] border-white border-opacity-0 hover:bg-white hover:bg-opacity-100" text="hover:!text-black">
					<x-slot name="icon">
		...
		<div class="container">
			<div class="flex flex-wrap items-center justify-between gap-8 pt-10 pb-7">
				<a href="{{route('index')}}">
				...
				<ul class="flex flex-wrap items-center gap-7 text-[14px]">
			...
			<hr class="border-white border-opacity-15">
			<div class="flex flex-wrap items-center justify-center gap-4 py-9">

				<p class="text-[14px] opacity-60 !text-end">{{$fSetting->footer_copyright}}</p>
			</div>
		</div>
	</div>
</footer>

					...
					@foreach($ticket->messages as $message)
						@if($message->sender == 'user')
							<div class="d-flex flex-wrap justify-content-end mb-2 !text-end lg:ms-auto lg:max-w-[50%]">
								<strong class="text-dark w-full block mb-1 group-[.theme-dark]/body:!text-white rtl:!text-end">{{$ticket->user->fullName()}}</strong>
								<div class="border-none rounded-[2em] mb-[7px] bg-[#F3E2FD] text-[#090A0A]">
									<div class="py-[0.75rem] px-[1.5rem]">
										{{$message->message}}
									</div>
								</div>
								<div class="w-full text-[11px] font-normal text-[rgba(0,0,0,0.4)] group-[.theme-dark]/body:!text-[rgba(255,255,255,0.7)] rtl:!text-end">
									{{$message->created_at}}
								</div>
							</div>
						@else

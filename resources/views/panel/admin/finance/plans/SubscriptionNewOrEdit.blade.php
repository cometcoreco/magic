...
@section('title', isset($subscription) ? 'Edit' : 'Create'.' Subscription')
						...
						<div class="row">
							<div class="col-md-12 col-xl-12">
									...
									<div class="col-md-6">
										<div class="mb-3">
											...
											<select class="form-select" id="frequency" name="frequency" required>
												<option value="{{isset($subscription) ? $subscription->frequency : null}}">{{isset($subscription) ? $subscription->frequency : 'Select'}}</option>
									...
									<div class="col-md-6">
										<div class="mb-3">
											...
											<select class="form-select" id="is_featured" name="is_featured" required>
												...
												<option value="{{$subscription->is_featured}}">{{$subscription->is_featured == 1 ? 'Yes' : 'No'}}</option>
												@endif
								...
								<div class="row">
									<div class="col-md-12">
										<div class="mb-3">
											<label class="form-label">{{__('Stripe Price Id')}}</label>
											<input type="text" name="stripe_product_id" id="stripe_product_id" class="form-control"  value="{{isset($subscription) ? $subscription->stripe_product_id : null}}"/>
										</div>
									</div>
									<div class="col-md-6">
										<div class="mb-3">
									...
									<div class="col-md-6">
										<div class="mb-3">
											...
											<select class="form-select" id="can_create_ai_images" name="can_create_ai_images" required>
												@if(isset($subscription))
													<option value="{{$subscription->can_create_ai_images}}">{{$subscription->can_create_ai_images == 1 ? 'Yes' : 'No'}}</option>
												@endif
												<option value="1">{{__('Yes')}}</option>
												<option value="0">{{__('No')}}</option>
											</select>
									...
									<div class="col-md-6">
										<div class="mb-3">
											...
											<select class="form-select" id="plan_type" name="plan_type" required>
												@if(isset($subscription))
													<option value="{{$subscription->plan_type}}">{{$subscription->plan_type}}</option>
												@endif

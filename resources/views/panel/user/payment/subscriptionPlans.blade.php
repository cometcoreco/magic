					...
					<div class="w-full border-solid border-[--tblr-border-color] rounded-3xl {{$plan->is_featured == 1 ? 'shadow-[0_7px_20px_rgba(0,0,0,0.04)]' : ''}}">
						<div class="flex flex-col h-full p-[1rem]">
							<div class="flex items-start mb-2 text-[50px] leading-none text-heading font-bold">
								{{$plan->price}}
								<div class="inline-flex flex-col gap-2 mt-2 ms-2 text-[0.3em]">
							...
							<div class="mt-auto -mx-[1rem] -mb-[1rem] text-center">
								@if($setting->stripe_key != null and $plan->stripe_product_id != null)
								<a class="btn rounded-3xl py-[0.75rem] -mx-px -mb-px w-full border border-[--tblr-border-color] text-[15px] font-semibold shadow-none hover:bg-[--tblr-primary] hover:text-white" href="{{ LaravelLocalization::localizeUrl( route('dashboard.user.payment.subscription.payment', $plan->id) ) }}">{{__('Choose plan')}}</a>
								@else
								<p>{{__('Please fill the stripe settings')}}</p>
								@endif
							</div>
						</div>
					</div>

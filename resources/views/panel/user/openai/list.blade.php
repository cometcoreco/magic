		...
		<div class="row g-2 items-center">
			<div class="col">
				...
				<ul class="flex flex-wrap items-center m-0 p-0 list-none text-[13px] text-[#2B2F37] gap-[20px] max-sm:gap-[10px]">
					<li>
						<button data-filter-trigger="all" class="inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-[#f2f2f4] transition-colors [&.active]:bg-[#f2f2f4] group-[.theme-dark]/body:text-[--tblr-muted] group-[.theme-dark]/body:[&.active]:bg-[--lqd-faded-out] group-[.theme-dark]/body:[&.active]:text-[--lqd-heading-color] active">
							All
						</button>
					</li>
					@foreach($filters as $filter)
					<li>
						<button data-filter-trigger="{{$filter->name}}" class="inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-[#f2f2f4] transition-colors [&.active]:bg-[#f2f2f4] group-[.theme-dark]/body:text-[--tblr-muted] group-[.theme-dark]/body:[&.active]:bg-[--lqd-faded-out] group-[.theme-dark]/body:[&.active]:text-[--lqd-heading-color]">
...
<div class="page-body mt-2 relative after:h-px after:w-full after:bg-[var(--tblr-body-bg)] after:absolute after:top-full after:left-0 after:-mt-px">
	<div class="container-fluid">
		<div class="row">
			...
			<div data-filter="{{$item->filters}}" class="col-lg-4 col-xl-3 col-md-6 pt-8 pb-10 px-16 relative border-b border-solid border-t-0 border-s-0 border-[var(--tblr-border-color)] group max-xl:px-10">
				...
				<div class="absolute top-0 left-0 w-full h-full transition-all z-2">
					@if($item->type == 'text' and Auth::user()->remaining_words > 0)
					...
					@elseif($item->type == 'image' and Auth::user()->remaining_images > 0)
					...
					@elseif($item->type == 'audio' and Auth::user()->remaining_words > 0)
						<a href="{{ LaravelLocalization::localizeUrl( route('dashboard.user.openai.generator', $item->slug)) }}" class="inline-block w-full h-full absolute top-0 left-0 overflow-hidden -indent-[99999px]">
							{{__('Create')}}
						</a>
					@elseif($item->type == 'code' and Auth::user()->remaining_words > 0)
						<a href="{{ LaravelLocalization::localizeUrl( route('dashboard.user.openai.generator.workbook', $item->slug)) }}" class="inline-block w-full h-full absolute top-0 left-0 overflow-hidden -indent-[99999px]">
							{{__('Create Workbook')}}
						</a>
					@else

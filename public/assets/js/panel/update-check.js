( () => {
	...
	$( document ).ready( function () {
		...
		if ( localStorage.getItem( "magicai_update_badge" ) ) {
			$( '.nav-link--update' ).append( localStorage.getItem( "magicai_update_badge" ) );
		} else {
			if ( !cache ) {
				$.ajax( {
					type: 'GET',
					...
					success: function ( response ) {
						if ( response != '' ) {
							let html = '<span class="text-xs rounded-full flex justify-center items-center bg-[#F3E2FD] !h-5 !w-5">1</span>';
							$( '.nav-link--update' ).append( html );
